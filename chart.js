...
var zoomLevel = 60; // 60% is the default zoom level

function zoomIn() {
  zoomLevel += 10;
  if (zoomLevel > 100) {
    zoomLevel = 100; // Limit maximum zoom to 100%
  }
  applyZoom();
  adjustFontSize(-10); // Decrease font size when zooming in
}

function zoomOut() {
  zoomLevel -= 10;
  if (zoomLevel < 50) {
    zoomLevel = 50; // Limit minimum zoom to 50%
  }
  applyZoom();
  adjustFontSize(10); // Increase font size when zooming out
}
...
function adjustFontSize(change) {
  ...
  var newFontSize = fontSize + change + 'px';
...
// Apply the default zoom level when the page loads
applyZoom();
